...
<!-- Hero Section -->
<div class="bg-gradient-to-br from-[#0f172a] to-slate-800 relative overflow-hidden">
  <i class="fas fa-truck text-white/5 text-[12rem] absolute -right-10 -bottom-10 rotate-[-8deg]"></i>
  <div class="max-w-6xl mx-auto px-4 sm:px-6 py-20 sm:py-28 relative z-10 text-center">
    <h1 class="text-4xl sm:text-5xl font-black text-white tracking-tight leading-tight animate-fade-in-up">
      Fresh Food,<br class="sm:hidden"> Straight From the Truck
    </h1>
    <p class="mt-4 text-lg text-slate-400 max-w-xl mx-auto animate-fade-in-up" style="animation-delay:.08s">
      Browse food trucks near you, explore their menus, and order your favourites.
    </p>
    <a href="#trucks"
       class="inline-flex items-center gap-2 mt-8 px-6 py-3 bg-amber-400 hover:bg-amber-500 text-amber-900 font-black text-sm rounded-2xl shadow-lg transition-all animate-fade-in-up" style="animation-delay:.16s">
      <i class="fas fa-store"></i> Browse Trucks
    </a>
  </div>
</div>

<!-- Truck Grid -->
<div id="trucks" class="max-w-6xl mx-auto px-4 sm:px-6 py-12">

  <div class="flex items-center justify-between mb-8 animate-fade-in-up">
    <div>
      <h2 class="text-2xl font-black text-gray-900 tracking-tight">Food Trucks Online</h2>
      ...
      @foreach($trucks as $truck)
        <a href="{{ url('/trucks/' . $truck->id) }}"
          ...
          <div class="h-28 bg-gradient-to-br from-[#0f172a] to-slate-700 flex items-center justify-center relative overflow-hidden">
            <i class="fas fa-truck text-5xl text-white/20 absolute -right-4 -bottom-2 scale-150"></i>
          ...
          <div class="p-5 flex flex-col flex-1">
            ...
            @if($truck->foodtruck_desc)
              <p class="text-xs text-gray-500 leading-relaxed flex-1">{{ Str::limit($truck->foodtruck_desc, 100) }}</p>
            @else
